...
| --- | --: | --- |
| `MAX_FETCHES` | all | Batch size: with a positive value the updater continues after the committed cursor in `api/invesco/update-state.json`; empty or `0` is a full pass — every fund is refreshed in one run. |
...
| `TICKERS` | all | Space-, comma- or semicolon-separated ticker allowlist, e.g. `QQQ QQQM RSP PGX`. |
...
TICKERS="QQQ QQQM RSP PGX" ./scripts/update-data.ts
