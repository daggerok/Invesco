...
| Invesco ETF Holdings to Watchlist | investco.com CSV downloads + Yahoo Finance | [daggerok/Invesco](https://github.com/daggerok/Invesco) · [published app](https://daggerok.github.io/Invesco/) |
...
| Holdings fallback (funds with no investco.com holdings CSV) | SEC EDGAR Form **N-PORT-P** of the Invesco ETF registrant, discovered through `efts.sec.gov` full-text search (no hand-kept CIK table) |
...
- **Holdings are the latest published snapshot only** — investco.com ships no historical holdings archive, so the feed has one sheet per fund, exactly like the SPDR feed.
...
| `REQUEST_SLEEP` | `1` | Minimum delay in seconds between outgoing request starts, including retries. investco.com and Yahoo throttle bursty clients; keep ≥ 1. |
...
| `AUDIENCE_TYPE` | `Investor` | investco.com `audienceType` query parameter (`Investor` or `Advisor`). |
...
| `CATALOG_HTML_URL` | ETF page | investco.com page scraped for canonical per-fund URLs (`?ticker=` links are the fallback). |
...
| `SKIP_YAHOO` | off | Update investco.com data only, keeping previously published history/distributions (also disables live ticker resolution). |
| `SKIP_INVESCO` | off | Update history only (Yahoo), keeping the published catalog values and holdings — useful when investco.com is down and only prices moved. |
...
- `scripts/update-data.ts` — Bun updater, zero runtime dependencies (`node:fs/promises` + `fetch` only): a tolerant CSV reader for the three investco.com download flavors, a forgiving N-PORT-P XML reader (same hand-rolled spirit as SPDR's workbook reader and Fidelity's EDGAR layer), Yahoo chart reader, derived-metric helpers (`annualizedToTotal`, `totalToAnnualized`, `indicatedYield`, `priceReturns`, `inferDistributionFrequency`, `deriveCatalogMetrics`), strict range parsers, bounded-run cursor, retries with 403/429 back-off, deterministic content-only writes.
