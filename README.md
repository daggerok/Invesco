...
- `scripts/ui-contract.test.ts` — `bun test` acceptance suite (happy-dom): boots the real `app.tsx` against the real `api/invesco` feed through a fetch stub and drives it through DOM events — per-tab sort persistence, filtered select-all scopes, reactive Watchlist aggregation (with race and latency injection), detail-tab data states, identifier fallbacks, sticky columns and malformed-localStorage boot safety. Expected Watchlist values are computed independently from the feed files.
...
The interaction rules below are shared with the sibling applications (documented in [`docs/ui-contract.md`](./docs/ui-contract.md)):

- **Per-tab sort persistence.** Every tab remembers its own last explicitly chosen column and direction, stored in browser localStorage under `invesco-tab-sorts` and reapplied whenever the tab is reopened — including after a full reload. **No button or checkbox resets a sort**: row Use checkboxes, the header select-all, the All ETFs pill checkbox, tab buttons, search, Copy Tickers, CSV/TXT export, the theme toggle, blacklist actions and **Clear** all keep it. Clear removes only the selection and the searches. A tab that was never explicitly sorted keeps its default order (catalog: Ticker asc; Watchlist: Weight Sum desc; Overview: Section asc; sheets: source order). To return to the default catalog order, click the *Ticker* header (asc).
- **Two select-all scopes.** The **Use** checkbox in the table header toggles only the rows currently rendered — current catalog/category tab + active search filter + blacklist exclusion: with a filter active it selects exactly the filtered ETFs, and unchecking it leaves selections made under other filters untouched. The checkbox inside the **All ETFs pill** always toggles the whole catalog (every non-blacklisted ETF), regardless of the active category/detail/Watchlist tab or search filter, and never navigates away from the current tab.
- **Reactive selection.** Every selection change immediately updates the selected count and clickable ticker badges in the subtitle, the active fund, the detail-tab counts and panel, and the Watchlist count. Clicking a catalog ticker, a selected-ticker badge or a Watchlist ETF badge opens that fund's details. Selection and active fund persist to localStorage and recover after reload (with background holdings loading — the Watchlist rebuilds without touching a checkbox).
- **Watchlist aggregation.** The Watchlist loads every holdings page of every selected ETF, deduplicating positions by **Ticker → Identifier (CUSIP/ISIN/SEDOL) → published name** (the name keeps loan/cash/derivative rows that legitimately have no identifier), and shows Ticker, Name, ETF badges, # ETFs, Weight Sum, Max Weight, Market Value, Asset Category and Identifier columns. While files load the tab shows `Watchlist (Loading…)` / `Watchlist (N+)` instead of a misleading exact count; once done the count is exact. Large all-catalog Watchlists render in 500-row scroll chunks while Copy/Export still use the complete filtered result.
- **Race-free loading.** Per-`meta.json` and per-page request deduplication plus a single page-writer per ticker prevent duplicate/skipped holdings pages when selection changes quickly (including deselect/reselect); detail views and Watchlist background loading share one cache; all-catalog loads run with bounded concurrency (8) and coalesced UI refreshes.
- **Missing data states.** Funds without a workbook, funds whose manifest has no pages, and fetch failures each show a specific explanatory state (never "no search matches"); a Watchlist search that matches nothing shows a search-specific empty state.
- **Sticky columns.** During horizontal scrolling the catalog **Use** and **Ticker** cells stay pinned (sticky positioning on each `th`/`td`, opaque pre-blended light/dark backgrounds), and the Watchlist **Ticker** column is pinned at the left edge.
